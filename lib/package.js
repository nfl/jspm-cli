  ...
  .then(function(dirInfo) {
    ...
    .then(function(_fresh) {
      ...
      .then(function(cacheInfo) {
        ...
        .then(function(packageConfig) {
          return _pkg.createMain(pkg, packageConfig, cacheDir)
          .then(function() {
            return packageConfig;
          });
        })
...
*/
exports.processPackage = function(pkg, dir, packageConfig, postload, isCDN) {
  ...
  return Promise.resolve(endpoint.processPackage && endpoint.processPackage(packageConfig, dir) || packageConfig)
  .catch(function(e) {
  ...
  .then(function(packageConfig) {
    ...
    if (!isCDN) {
      ...
      deps = packageConfig.dependencies;
      packageConfig.dependencies = processDeps(packageConfig.dependencies, packageConfig.registry);
    }

    return build.buildPackage(dir, packageConfig, isCDN);
  })
  .then(function(_buildErrors) {
    if (_buildErrors)
      buildErrors = _buildErrors;
    packageConfig.dependencies = deps;
  })
  .then(function() {
    // write build errors
...
exports.createMain = function(pkg, packageConfig, downloadDir) {
  ...
  .then(function() {
    ...
    main = typeof packageConfig.main === 'string' && packageConfig.main;
  ...
  .then(function(exists) {
    ...
    if (!exists) {
      if (packageConfig.main !== false)
        ui.log('warn', 'Main entry point not found for `' + pkg.exactName + '`.\nAdjust this property in the package config or with an override, setting %"main": false% if this is the intention.\n');
      return;
    }
